...
- 🧹 **自动清理**: 定期清理过期会话和失效账号
- ⚡ **429错误处理**: 自动删除触发429的账号，立即切换新账号重试
- 🔗 **IP会话绑定**: 同一IP地址自动复用相同账号，提高稳定性
...
- ⚡ **并发优化**: 连接池和响应缓存优化
- 🔁 **智能重试**: 429错误时自动切换账号，最多重试MAX_429_RETRY_LIMIT次
...
### 3. 429错误处理流程（新增 v1.1.0）
```
1. Warp API 返回 429 错误 → OpenAI Agent 检测
2. 立即释放并删除当前账号 → 账号池 Agent
3. 账号池 Agent → 分配新的可用账号
4. 使用新账号立即重试 → 无需等待
5. 达到重试上限 → 返回最终错误
6. 账号池自动补充 → 维持健康水平
```

### 4. IP会话绑定流程（新增 v1.2.0）
```
1. 客户端请求 → OpenAI Agent 提取 IP 地址
2. 基于 IP 生成稳定 session_id → MD5(IP)[:16]
3. 账号池 Agent 检查会话 → 是否已有绑定
4. 已绑定 → 返回已有账号，更新活跃时间
5. 未绑定 → 分配新账号并绑定到 session_id
6. 后续请求 → 同一 IP 复用相同账号
7. 超时释放 → SESSION_TIMEOUT 秒后自动释放
```
...
### 429错误专项指标（新增 v1.1.0）
- **total_429_errors**: 总429错误次数
- **deleted_accounts**: 因429删除的账号数
- **hourly_deletes**: 每小时删除账号数
- **retry_success_rate**: 重试成功率
- **last_429_time**: 最后一次429错误时间

### IP会话绑定指标（新增 v1.2.0）
- **active_sessions**: 当前活跃会话数
- **session_reuse_count**: 会话复用次数
- **unique_ips**: 唯一IP数量
- **avg_session_lifetime**: 平均会话生命周期
- **session_timeout_count**: 超时释放的会话数
...
POOL_SERVICE_URL=http://localhost:8019 # 账号池地址
